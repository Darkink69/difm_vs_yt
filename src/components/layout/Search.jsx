...
class Search extends React.Component {
    ...
    handleKey = (event) => {
        if (event.key === 'Enter') {
            this.props.searchText(this.state.search, this.state.type)
        }
    }
    ...
    otherFilters = (event) => {
        this.setState({nine: event.target.dataset.type})
    }
    ...
    render() {
        return <div className="row">
            ...
            <div>
                ...
                <label>
                    <input className="with-gap" name="group3" type="radio" data-type='90x' onChange={this.otherFilters}/>
                    <span>90x</span>
                </label>
            </div>
        </div>
